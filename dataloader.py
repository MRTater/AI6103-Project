...
# Batch based calculation of Mean & Std for Normalization
def calculate_mean_and_std(dataloader):
    ...
    for images, _ in dataloader:
        for c in range(3):  # for R,G,B
            means[c] += images[:, c, :, :].mean()
            stds[c] += images[:, c, :, :].std()

    # Compute the overall mean and std
    mean = means / len(dataloader)
    std = stds / len(dataloader)
    return mean, std
...
def load_transformed_dataset(dataset_folder, img_size, bs, num_workers=4):
    ...
    data = ImageDataset(dataset_folder, transform=data_transform)
