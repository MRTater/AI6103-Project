...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.45\textwidth}
        ...
        \caption{64*64, T=300, cosine} % Replace with the correct caption
        \label{fig:figure4b}
    \end{subfigure}
...
% The forward variances are a sequence of linearly increasing constants from $\beta_1=10^{-4}$ to $\beta_T=0.02$. We perform both linear and cosine $\beta$ schedules to investigate their impact on the model's performance. 

% We set the maximum value of $\beta$ for the cosine schedule to be the same as that for the linear schedule. Our results showed that the cosine schedule outperformed the linear schedule in terms of sample quality and training stability. Specifically, the cosine schedule produced lower FID scores compared to the linear schedule.

% In conclusion, we found that the cosine $\beta$ schedule is a more effective approach for training our diffusion model than the linear schedule. Its smoother and more gradual transition between values provides improved performance and stability during training.
